...
def update_db():
    area_list = get_areas('area2.json')
    for i in range(2014, 2020):
        ...
        for j in area_list:
            ...
            if (i == 2019) and (j == "bayside"):
                data['total_positive_twitter_count'] = int(t_positive/100)
                data['total_negative_twitter_count'] = int(t_negative/100)
                data['total_twitter_post'] = int(t_total/100)
            else:
...
def get_positive_all(area):
    ...
    for i in range(2014, 2020):
        ...
        url = "http://user:pass@172.26.38.189:5984/t_" + str(i) + "/" + area
...
def get_positive(date, area):
    url_part1 = "http://user:pass@172.26.38.189:5984/positive_tweets_" + date
...
def get_negative(date, area):
    url_part1 = "http://user:pass@172.26.38.189:5984/negative_tweets_" + date
    url_part2 = "/_design/count/_view/new_view?group_level=1"
    url = url_part1 + url_part2
    http = urllib3.PoolManager()
    request = http.request('GET', url)
...
def get_neutral(date, area):
    url_part1 = "http://user:pass@172.26.38.189:5984/neutral_tweets_" + date
    url_part2 = "/_design/count/_view/new_view?group_level=1"
    url = url_part1 + url_part2
    http = urllib3.PoolManager()
    request = http.request('GET', url)
...
def get_score(date, area):
    ...
    sun_score = 0
    for i in db_list:
        url_part1 = "http://user:pass@172.26.38.189:5984/" + i + "_tweets_" + date
